...
class type_invoice_email_cc(models.TransientModel):
    _name = 'type_invoice.email_cc'

    contact = fields.Boolean(string="Contact")
    invoice = fields.Boolean(string="Adresse de facturation")
    delivery = fields.Boolean(string="Adresse d'expédition")
    other = fields.Boolean(string="Autre Adresse") 
    private = fields.Boolean(string="Adresse personnelle")
...
class MailComposer(models.TransientModel):
    ...
    def _get_default_cc_ids(self):
        ...
        doc_type = self.env.context.get('default_model')

        if doc_type =='account.invoice':

            doc_id = self.env.context.get('active_id')
            part = self.env['account.invoice'].search([('id','=', doc_id)])
            part_ids = self.env['res.partner'].search([('parent_id', '=', part.partner_id.id)])
            type_invoice = self.env['type_invoice.email_cc'].search([])
            tab_id = []
            type_adress_mail = []
            ...
            if type_invoice[-1].contact:
                type_adress_mail.append('contact')

            if type_invoice[-1].delivery:
                type_adress_mail.append('delivery')

            if type_invoice[-1].invoice:
                type_adress_mail.append('invoice')

            if type_invoice[-1].other:
                type_adress_mail.append('other')

            if type_invoice[-1].private:
                type_adress_mail.append('private')     

            if len(type_adress_mail) > 0:
                result = self.env['res.partner'].search([('id', 'in', tab_id), ('type','in', type_adress_mail)])
                return result
        else:
